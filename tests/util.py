...
class EncoderMixIn(object):
    ...
    def setUp(self):
        self.encoder = miniamf.get_encoder(encoding=self.amf_type)
...
class DecoderMixIn(object):
    ...
    def setUp(self):
        self.decoder = miniamf.get_decoder(encoding=self.amf_type)
...
class ClassCacheClearingTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self._class_cache = miniamf.CLASS_CACHE.copy()
        self._class_loaders = copy.copy(miniamf.CLASS_LOADERS)

    def tearDown(self):
        unittest.TestCase.tearDown(self)

        miniamf.CLASS_CACHE = self._class_cache
        miniamf.CLASS_LOADERS = self._class_loaders
